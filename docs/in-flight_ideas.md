...
The double parent walk in `_check_write_target` (the containment check walks the components once, the file-or-folder check walks them again) and the scope test's per-route setup cost. Whether either is worth a change of its own, or waits for a change that touches its code.
