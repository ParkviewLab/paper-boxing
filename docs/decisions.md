...
Decided by the owner against a live preview of the header: the label beside the logo is set in Michroma, the brand's wordmark face, the name `paper-boxing` at 21 px and the version `v<version>` at 12 px, the version sitting on the name's baseline and separated from it by a real space character, so that the label's text still reads `paper-boxing v<version>` to a test and a screen reader, and by a gap of .5em; white, opacity .85 and letter-spacing .02em stay as they were, for both parts. The face comes from the vendored woff2, embedded as data in one `@font-face` in the page head built by the same function that embeds it in the logo, never imported from the network and never a second copy of the file. The logo and the label cannot shrink in the header's row, so that when space runs out the row wraps, as before, and the logo renders at its full 60 px height and natural width at every width. The sign-in page, which has no header, is unchanged. Reason: the sizes, the baseline, the gap and the fixed logo are the owner's choices, made against the live preview. Consequence: the second, smaller span for the version, which the entry of 2026-09-17 on the version in the header allowed and did not take, is now taken. Set aside: none recorded.

Carried out as decided. Measured in Chromium against the stack built from the tree, with the development version `v0.2.1.dev0`: the logo renders at 167 by 60 px at 1440, 1024, 800 and 320 px; the name and the version share one baseline; the width below which the header wraps to more than one row moves from 876 px to 992 px, with no horizontal overflow at any of the widths measured, from 320 px up.
