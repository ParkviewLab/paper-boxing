...
Decided, in conversation: the site server serves source code, configuration, data and plain-text files as `text/plain` with `charset=utf-8` (Python, TypeScript and JSX, Rust, C and C++, Swift and Objective-C, other languages, shell and build files, configuration and data, documents and text, schemas and interface definitions, diagram sources, XML schemas and tooling, and robotics formats; the `types` block of the compose file is the list, and `tests/_site_server_types.py` holds it to those groups), `.cjs` as `application/javascript` beside `.mjs`, and a file with no extension or with an extension in no table as `text/plain` too (`default_type`), with the charset confined to `text/plain` so that no page, style sheet or script gains a charset header that could override its own declaration; `html`, `htm`, `css`, `js`, `mjs`, `svg`, `wasm`, `json`, `xml`, `webmanifest`, `map` and the stock table's images, fonts and binaries stay as they were, as do the listing, `Cache-Control: no-cache` and read-only serving. Reason: nginx's stock `default_type application/octet-stream` answers for every extension its table lacks, which makes the browser download a file a person wanted to read; a design site holds scripts, specifications and configuration beside its pages, and a `README`, `LICENSE`, `Makefile` or `Dockerfile` has no extension. The bytes served are unchanged; only the header is, so the page as served stays the page as uploaded. Accepted cost, as recorded: a binary file with an unknown extension shows as garbage instead of downloading. Set aside: keeping the stock table, with a download for everything it does not know; a charset on every text type, which could override a page's own declaration.

Carried out with two departures, made in implementation and put to the owner with the pull request; the merge is their acceptance, and a later entry records any change. The decision named `charset_types` to confine the charset; on nginx 1.30.5 the charset module adds the charset to `text/html` beside the types `charset_types` names, so every page, the listing and the 404 page would have carried one, contrary to the confinement decided; the charset is therefore written into the type itself, in the `types` block and in `default_type`, and no `charset` directive is set. The decision lists `map` among the types to stay unchanged, but the stock table has no `map`, so with nothing said it would have fallen to the default type; the block gives `map` `application/octet-stream` explicitly, which is what it answered before, the literal reading of unchanged; `application/json`, which other servers answer for a source map, is not what the decision names and was not chosen. `txt`, `ts`, `pl` and `pm` override the stock table's `text/plain`, `video/mp2t` and `application/x-perl` on purpose. Two costs the decision does not state follow from it: the stock table also lacks `ttf`, `otf`, `gz`, `tgz`, `tar`, `wav`, `sqlite` and `pyc`, so those are among the files now served as text (a browser loads a font whatever the type; the rest are the garbage the accepted cost describes); and a text file not in UTF-8, such as a Windows-1252 `.txt` or a UTF-16 `.csv`, displays with wrong characters where before the browser inferred a `.txt`'s encoding and downloaded the rest.
