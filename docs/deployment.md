...
[`docker-compose.yml`](../docker-compose.yml) is a copy-and-edit example. Every setting is a `${...}` variable read from a `.env` file beside it or from Portainer's stack environment; the two secrets have no default, and compose refuses to start until they are set. [`.env.example`](../.env.example) lists them all, and every default is in the README's Configuration table; this section says what each variable is for.

`backend` publishes 35843 and mounts the data volume at `/data`. Its variables: `PAPER_BOXING_PUBLIC_SITES_URL` (the site server's address as people and links reach it; every site's URL is built from it), `PAPER_BOXING_ADMIN_USERNAME` and `PAPER_BOXING_ADMIN_PASSWORD` (the first account, created only when no account exists; ignored afterwards; the password is a secret), `PAPER_BOXING_MAX_UPLOAD_MB` (the size cap of one file) and `PAPER_BOXING_SESSION_DAYS` (the sliding expiry of a UI session).

`frontend` publishes 35840 and reaches the backend by its service name on the compose network. Its secret is `PAPER_BOXING_STORAGE_SECRET`, a long random string (`openssl rand -hex 32`) that signs the per-user session cookie; changing it signs everyone out.

`mcp` publishes 35842 and reaches the backend the same way. `PAPER_BOXING_MCP_ALLOWED_HOSTS` is the list of `Host` values agents connect with; a LAN deployment adds its own host on the MCP port (DNS-rebinding protection: a request with another Host gets 421). `PAPER_BOXING_MCP_ALLOWED_ORIGINS` is for browser-based clients only; a non-browser MCP client sends no Origin and passes. `PAPER_BOXING_MCP_MAX_FILE_MB` caps the file a tool call carries; larger files go through the REST API.
