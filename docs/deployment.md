...
What the directives do: `index index.html` serves a site's or a folder's `index.html` at its address; `autoindex on` (with `autoindex_exact_size off` and `autoindex_localtime on`) makes nginx list a folder that has none, which is what one wants of a site of a few hand-made pages before it has an index; `try_files $uri $uri/ =404` keeps a missing path a 404; `add_header Cache-Control "no-cache"` makes a replaced file show at once (the browser revalidates through the `ETag` on every request); `types`, placed after the `include` of the stock `mime.types` in the same block so that it extends the table rather than replacing it, adds `application/javascript` for `.mjs` and `.cjs`, `application/manifest+json` for `.webmanifest`, and `text/plain; charset=utf-8` for source code, configuration, data and plain-text files (`.py`, `.ts`, `.rs`, `.c`, `.go`, `.sh`, `.yaml`, `.toml`, `.txt`, `.md`, `.proto`, `.xsd`, `.urdf` and their kin; the whole list, by group, is `tests/_site_server_types.py` and the block itself), so that the browser displays them instead of downloading them as `application/octet-stream`, which the stock table answers for an extension it does not know; and `default_type "text/plain; charset=utf-8"` does the same for a file with no extension or with an extension in no table (`README`, `LICENSE`, `Makefile`, `Dockerfile`, an extensionless script), at the accepted cost that a binary file with an unknown extension shows as garbage instead of downloading. The charset is written into the type itself, not set with nginx's `charset` directive: `charset_types` always includes `text/html` beside the types it names, so `charset utf-8` would give every page, the listing and the 404 page a `charset` parameter that could override a page's own declaration, whereas a type written with its charset is emitted as written and nothing else gains one. The bytes served are unchanged by all of this; only the `Content-Type` header is. Four entries (`txt`, `ts`, `pl`, `pm`) override the stock table's `text/plain`, `video/mp2t` and `application/x-perl` on purpose, and nginx reports each as a `duplicate extension` warning at start-up, which `nginx -t` passes. (In the compose file the `$` of `$uri` is written `$$`, which is how a literal dollar is spelt there.)
