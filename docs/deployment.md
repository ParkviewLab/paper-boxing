...
| `PAPER_BOXING_MCP_ALLOWED_HOSTS` | mcp | no | the `Host` values with which agents connect, comma-separated; `<host>:*` matches any port. A request with another Host gets 421 (DNS-rebinding protection). The file's default keeps the loopback names and adds a `CHANGE-ME:35842` placeholder |
...
- A `413` from the MCP server with code `payload_too_large`: the request's declared `Content-Length` is above the endpoint's body limit, which the message states in bytes (11,544,686 at the default cap). The file goes through the REST API instead, as described under the agent token above.
