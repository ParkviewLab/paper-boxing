...
- A source, configuration or text file, or a file with no extension, downloads instead of displaying in the browser, and `curl -I http://<host>:35841/<slug>/<file>` answers `Content-Type: application/octet-stream`: the stack runs an older nginx configuration, from before the text types were added. Bring the deployment's compose file up to this repository's and redeploy the `sites` service; `docker exec paper-boxing-sites nginx -T | grep -F 'default_type "text/plain'` prints `default_type "text/plain; charset=utf-8";` once the configuration is current, and nothing before (the stock `nginx.conf`'s own `default_type application/octet-stream` does not match the pattern).
