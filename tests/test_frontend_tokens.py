...
async def test_secret_is_shown_exactly_once(user: User, fake_state: FakeState) -> None:
    ...
    (secret,) = _agent_secrets(fake_state)
    assert secret.startswith("pb_")
    assert user.find(marker="token-secret").elements.pop().value == secret
    assert fake_state.tokens[fake_state.secrets[secret]].scope is Scope.READ_WRITE
...
async def test_session_token_is_never_shown(user: User, fake_state: FakeState) -> None:
    await support.sign_in(user, *support.ADMIN, at="/tokens")
    user.find(marker="token-name").type("t")
    user.find(marker="create-token").click()
    await user.should_see("Token 't' created")
    (session,) = support.session_secrets(fake_state, support.ADMIN[0])
    for path in ("/tokens", "/", "/users", "/account"):
        await user.open(path)
        assert session not in support.page_text(user), path


async def test_revoke(user: User, fake_state: FakeState) -> None:
    token, secret = fake_state.issue_agent_token(support.ADMIN[0], "old agent", Scope.READ_ONLY)
    await support.sign_in(user, *support.ADMIN, at="/tokens")
    ...
    await user.should_see(marker="no-tokens")
    assert fake_state.tokens[token.id].revoked is True


async def test_scope_is_explained_and_read_only_is_the_default(user: User, fake_state: FakeState) -> None:
    await support.sign_in(user, *support.ADMIN, at="/tokens")
    ...
    (secret,) = _agent_secrets(fake_state)
    assert fake_state.tokens[fake_state.secrets[secret]].scope is Scope.READ_ONLY
