...
async def test_change_password(user: User, frontend_state: FakeState) -> None:
    other_session = frontend_state.issue_session(support.ADMIN[0])  # another browser, signed in earlier
    ...
    assert frontend_state.user_by_name(support.ADMIN[0]).password == "new-password-1"
    assert frontend_state.tokens[frontend_state.secrets[other_session]].revoked is True
    assert len(support.session_secrets(frontend_state, support.ADMIN[0])) == 1  # this one survives
    await user.open("/")
    await user.should_see(marker="current-user")


async def test_wrong_current_password_is_refused(user: User, frontend_state: FakeState) -> None:
    await support.sign_in(user, *support.ADMIN, at="/account")
    ...
    assert frontend_state.user_by_name(support.ADMIN[0]).password == support.ADMIN[1]


async def test_checks_on_the_page(user: User, frontend_state: FakeState) -> None:
    await support.sign_in(user, *support.ADMIN, at="/account")
    user.find(marker="current-password").type(support.ADMIN[1])
    ...
    assert frontend_state.user_by_name(support.ADMIN[0]).password == support.ADMIN[1]
    assert not [r for r in frontend_state.requests if r.route == "change_password"]
